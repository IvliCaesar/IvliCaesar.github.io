...
\author{César Galindo López\\ \normalsize Notas de Preparación para Examen General}
\date{\normalsize Fecha: Diciembre 2013}
...
\[
\begin{aligned}
[\overline\sigma(p\circ\alpha)\sigma]\in p_*\pi_1(X,y)&=p_*\pi_1(X,h(x))=p_*h_*\pi_1(X,x)\\
&=(p\circ h)_*\pi_1(X,x)=p_*\pi_1(X,x).
\end{aligned}
\]
...
\[
\begin{aligned}
H_q(X,A)\cong H_q(X,V)\cong H_q(X-A,V-A)&\cong H_q\big(X/A-A/A,V/A-A/A\big)\\
&\cong H_q(X/A,V/A)\cong H_q(X/A,A/A),
\end{aligned}
\]
...
\[
\begin{aligned}
\cdots\to H_{q+1}(X,A_1\cup B_1)\to H_q(A\cap B,A_1\cap B_1)&\\
&\xrightarrow{(i_*,-j_*)}H_q(A,A_1)\oplus H_q(B,B_1)\\
&\xrightarrow{k_*+l_*}H_q(X,A_1\cup B_1)\\
&\to H_{q-1}(A\cap B,A_1\cap B_1)\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
\phi:S_\bullet(A\cap B)/S_\bullet(A_1\cap B_1)&\to \big(S_\bullet(A)/S_\bullet(A_1)\big)\oplus\big(S_\bullet(B)/S_\bullet(B_1)\big),\\
&\qquad c\mapsto(i_\#c,\,-j_\#c),
\end{aligned}
\]
\[
\begin{aligned}
\psi:\big(S_\bullet(A)/S_\bullet(A_1)\big)\oplus\big(S_\bullet(B)/S_\bullet(B_1)\big)&\to\big(S_\bullet(A)+S_\bullet(B)\big)/\big(S_\bullet(A_1)+S_\bullet(B_1)\big),\\
...
\[
\begin{aligned}
\cdots\to H_{q+1}\Big(\tfrac{S_\bullet(A)+S_\bullet(B)}{S_\bullet(A_1)+S_\bullet(B_1)}\Big)\to H_q(A\cap B,A_1\cap B_1)&\\
&\xrightarrow{(i_*,-j_*)}H_q(A,A_1)\oplus H_q(B,B_1)\\
&\xrightarrow{\psi_*}H_q\Big(\tfrac{S_\bullet(A)+S_\bullet(B)}{S_\bullet(A_1)+S_\bullet(B_1)}\Big)\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
0=H_{n+1}(X,Y)\to\tilde H_n(Y)\to\tilde H_n(X)&\xrightarrow{\ \partial\ }H_n(X,Y)\\
&\xrightarrow{\ \partial\ }\tilde H_{n-1}(Y)\to\tilde H_{n-1}(X)\to H_{n-1}(X,Y)=0.
\end{aligned}
\]
...
\[
\begin{aligned}
\cdots\to H_q(X\cap A,A\cap B)\to H_q(X,B)\oplus H_q(A,A)&\to H_q(X,A\cup B)\\
&\to H_{q-1}(X\cap A,A\cap B)\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
\cdots\to H_1(A\cap B)&\xrightarrow{(i_*,j_*)}H_1(A)\oplus H_1(B)\\
&\xrightarrow{k_*-l_*}H_1(\mathbb T^2)\xrightarrow{\delta}H_0(A\cap B)\\
&\xrightarrow{(i_*,j_*)}H_0(A)\oplus H_0(B)\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
\cdots\to\tilde H_n(\mathcal H_n-\{x\})\to\tilde H_n(\mathcal H_n)&\to H_n(\mathcal H_n,\mathcal H_n-\{x\})\\
&\to\tilde H_{n-1}(\mathcal H_n-\{x\})\to\tilde H_{n-1}(\mathcal H_n)\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
F:(D^{2n},S^{2n-1})&\to(\CC P^n,\CC P^{n-1}), \\
...
\[
\begin{aligned}
\cdots\to H_q(\{*\},\{*\})\to H_q(A,\{*\})\oplus H_q(S_1,\{*\})&\to H_q(X,\{*\})\\
&\to H_{q-1}(\{*\},\{*\})\to\cdots
\end{aligned}
\]
...
\[
\begin{aligned}
i_1&:S^1\to S^1\vee S^1, \qquad z\mapsto(z,1),\\
...
\[
\begin{aligned}
H_0(\mathbb T^2)&=\mathcal C_0/\im d_1=\ZZ; \\
H_1(\mathbb T^2)&=\ker d_1/\im d_2\cong\mathcal C_1/\im d_2=\ZZ\oplus\ZZ; \\
...
\[
\begin{aligned}
0\to\cdots\to H_q(A;\ZZ)\to H_q(X;\ZZ)&\to H_q(X,A;\ZZ)\\
&\to H_{q-1}(A;\ZZ)\to\cdots\to H_0(X,A;\ZZ)\to0
\end{aligned}
\]
...
\[
\begin{aligned}
0&\to R\xrightarrow{2}R\xrightarrow{0}R\xrightarrow{2}\cdots\xrightarrow{0}R\xrightarrow{2}R\xrightarrow{0}R\to0;\\
...
\[
\begin{aligned}
H_0(\RR P^n)&=\ker d_0/\im d_1=R; \\
H_1(\RR P^n)&=\ker d_1/\im d_2=R/2R; \\
H_2(\RR P^n)&=\ker d_2/\im d_3=R_2; \\
...
\[
\begin{aligned}
H_{2j}(\RR P^{2j})&=\ker d_{2j}/\im d_{2j+1}=R_2; \\
...
&=\ran(H_0(\RR P^n;\ZZ))+\sum_{j=0}^{[\frac{n-1}2]-1}(-1)^{2j+1}\ran\big(H_{2j+1}(\RR P^n;\ZZ)\big)\\
&\qquad+(-1)\ran\big(H_{2j+1}(\RR P^n;\ZZ)\big)\\
...
en donde
\[
f_*:\tilde H_{2n-1}(S^{2n-1})\to\tilde H_{2n-1}(\CC P^{n-1})=0
\]
...
\[
\begin{aligned}
H_0(T_g)&=\ker d_0/\im d_1=R/0=R; \\
H_1(T_g)&=\ker d_1/\im d_2=R^{2g}; \\
...
\[
\begin{aligned}
H_0(P_g)&=\ker d_0/\im d_1=R; \qquad H_1(P_g)=\ker d_1/\im d_2=R^g/2R;\\
&H_2(P_g)=\ker d_2/\im d_3=R_2.
\end{aligned}
\]
En resumen,
\[
...
\[
\begin{aligned}
\cdots\to\tilde H_{q+1}(Z)\to\tilde H_q(Z_+\cap Z_-)\to\tilde H_q(Z_+)\oplus\tilde H_q(Z_-)&\to\tilde H_q(Z)\\
&\xrightarrow{\cong}\tilde H_{q-1}(Z_+\cap Z_-)\to0
\end{aligned}
\]
...
\[
\begin{aligned}
0\to\Hom(H_q(X;\ZZ),G)\to\Hom(Z_q,G)&\xrightarrow{\iota_q^*}\Hom(B_q,G)\\
&\to\mathrm{Ext}^1_\ZZ(H_q(X;\ZZ),G)\to\mathrm{Ext}^1_\ZZ(Z_q,G)=0
\end{aligned}
\]
...
Teorema~\ref{thm:cohomologia-lente} para
\[
H_q(L_m(\ell_1,\dots,\ell_n);\ZZ) \qquad\text{y}\qquad H^q(L_m(\ell_1,\dots,\ell_n);\ZZ)
\]
dependen únicamente de $m$ y de $n$
...
\[
\begin{aligned}
H^2&\cong\Hom(0,\ZZ)\oplus\mathrm{Ext}^1(\ZZ/m\ZZ,\ZZ)=0\oplus\ZZ/m\ZZ=\ZZ/m\ZZ,\\
...
\[
\begin{aligned}
H^0(K;\ZZ)&=\ZZ, \qquad H^1(K;\ZZ)=\ker(\delta^1)=\ZZ,\\
